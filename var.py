...
# Giả sử ở Bước 5, khi nhấn "Kết thúc var"
if st.button("Kết thúc var"):
    st.balloons()
    
    # 1. Tính toán số điểm ngẫu nhiên dựa trên mức độ đã chọn
    if st.session_state.muc_do == "Boy phố":
        diem = random.randint(80, 100)
    elif st.session_state.muc_do == "Phong cách Premium":
        diem = random.randint(50, 79)
    else: # Mức vừa, nhẹ nhàng
        diem = random.randint(10, 49)
        
    # 2. Xử lý phân hạng theo yêu cầu của bạn
    st.markdown("---")
    st.header(f"📊 Tổng kết điểm: {diem} điểm")
    
    if diem >= 80:
        st.error("🏆 HẠNG: CHIẾN THẦN LEO TOP")
        st.write("🔥 Khét quá! Bạn là trùm phân khối lớn trong làng giải Toán đêm nay.")
        st.markdown("![Chiến Thần](https://img.icons8.com/emoji/96/000000/fire.png)")
        
    elif 50 <= diem < 80:
        st.warning("⚔️ HẠNG: CHIẾN TƯỚNG LEO TOP")
        st.write("✨ Điểm số rất ổn định. Phong cách cực kỳ Premium, jet jet!")
        
    else:
        st.info("🐢 HẠNG: NGÔ LEO TOP")
        st.write("Chưa có điểm rực rỡ rồi. Cần nạp thêm 'xăng' kiến thức để bứt tốc nhé!")

    # Nút chơi lại để reset hệ thống
...
# --- BƯỚC 5: KẾT QUẢ ---
    ...
    # Tạo một vùng trống để hiển thị nội dung var
    vung_var = st.empty()
    ...
    with col4:
        if st.button("Kết thúc var"):
            st.balloons()
            vung_var.success("🏆 Kèo var rực rỡ! Bạn đã leo top bảng xếp hạng Toán học đêm nay.")
